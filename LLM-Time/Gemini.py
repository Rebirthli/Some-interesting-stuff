...
class GeminiPerformanceTester:
    ...
    def test_single_prompt(self, model, prompt):
        ...
        try:
            ...
            # 创建模型实例
            gemini_model = genai.GenerativeModel(model_name=model, generation_config=model_config)
            ...
            for chunk in response:
                ...
                # 检查是否是第一个token
                if not first_token_received and hasattr(chunk, 'text') and chunk.text:
                ...
                if hasattr(chunk, 'text') and chunk.text:
                    ...
                    # 粗略估计token数量，每个单词或标点符号算作一个token
                    total_tokens += len(content.split()) + len([c for c in content if c in ',.;:!?'])
            ...
            while retry_count < max_retries:
                ...
                try:
                    ...
                    if response and response.text:
                        ...
                        # 估算token数量
                        total_tokens = len(response_text.split()) + len([c for c in response_text if c in ',.;:!?'])
                        tokens_per_second = total_tokens / total_time if total_time > 0 else 0
                        normalized_latency = total_time / total_tokens if total_tokens > 0 else 0
                        
                        print(f"    非流式请求成功，估算tokens: {total_tokens}")
    ...
    def save_results(self):
        ...
        filename = os.path.join(self.output_dir, f"gemini_performance_test_{timestamp}.json")
...
def main():
    parser = argparse.ArgumentParser(description='Gemini模型性能测试工具')
    parser.add_argument('--models', nargs='+', default=DEFAULT_MODELS, 
                        help='要测试的模型列表，例如 gemini-1.5-pro gemini-1.5-flash')
    parser.add_argument('--runs', type=int, default=3, 
                        help='每个提示运行的次数')
    parser.add_argument('--prompts-file', type=str, 
                        help='包含测试提示的JSON文件路径')
    parser.add_argument('--output-dir', type=str, default='Gemini-results', 
                        help='结果输出目录')
    ...
    # 初始化测试器
    tester = GeminiPerformanceTester(
        models=args.models,
        prompts=prompts,
        runs=args.runs,
