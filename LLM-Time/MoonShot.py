...
client = OpenAI(
    api_key=api_key,
    base_url="https://api.moonshot.cn/v1"  # MoonShot API的基础URL
)
...
DEFAULT_MODELS = ["kimi-latest", "moonshot-v1-8k", "moonshot-v1-32k", "moonshot-v1-128k"]

class MoonShotPerformanceTester:
    ...
    def save_results(self):
        ...
        filename = os.path.join(self.output_dir, f"moonshot_performance_test_{timestamp}.json")
...
def main():
    parser = argparse.ArgumentParser(description='MoonShot模型性能测试工具')
    ...
    parser.add_argument('--output-dir', type=str, default='MoonShot-results', 
                        help='结果输出目录')
    ...
    # 初始化测试器
    tester = MoonShotPerformanceTester(
        models=args.models,
        prompts=prompts,
        runs=args.runs,
