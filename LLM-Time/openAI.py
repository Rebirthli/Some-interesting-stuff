...
#加载环境变量中的API密钥
load_dotenv()
api_key = os.getenv("OPENAI_API_KEY")  # OpenAI的API密钥
...
client = OpenAI(
    api_key=api_key,
    base_url="https://api.openai.com/v1"  # OpenAI的API基础URL
)
...
class OpenAIPerformanceTester:  # 修改类名以反映测试对象
        ...
        for model in self.models:
            ...
            for i, prompt in enumerate(self.prompts):
                ...
                for run in range(self.num_runs):
                    ...
                    time.sleep(2)  # OpenAI可能有更严格的速率限制，增加等待时间
    ...
    def test_single_prompt(self, model, prompt):
        ...
        try:
            ...
            stream = client.chat.completions.create(
                ...
                stream=True,
                timeout=90  # 增加超时时间，OpenAI可能需要更长时间
            )
    ...
    def save_results(self):
        ...
        filename = os.path.join(self.output_dir, f"openai_performance_test_{timestamp}.json")
...
def main():
    parser = argparse.ArgumentParser(description='OpenAI模型性能测试工具')
    parser.add_argument('--models', nargs='+', default=DEFAULT_MODELS, 
                        help='要测试的模型列表，例如 gpt-4o gpt-3.5-turbo')
    parser.add_argument('--runs', type=int, default=3, 
                        help='每个提示运行的次数')
    parser.add_argument('--prompts-file', type=str, 
                        help='包含测试提示的JSON文件路径')
    parser.add_argument('--output-dir', type=str, default='OpenAI-results', 
                        help='结果输出目录')
